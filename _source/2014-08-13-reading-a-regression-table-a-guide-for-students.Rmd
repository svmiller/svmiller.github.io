...
```{r, echo=F, eval=T}
# library(Zelig) # quietly run, check _source
# data(voteincome) # quietly run, check _source
summary(voteincome)
```
...
stargazer(M1, type="html",
          covariate.labels = c("Age","Female","Education","Income","Constant"),
          title="Explaining Registered Voters in 2000 (Arkansas, South Carolina)",
          notes="<small>Data come from <code>voteincome</code> data in <code>Zelig</code> package.</small>",
          dep.var.labels="Is Respondent a Voter?",
          omit.stat = c("aic","ll"))
```
<br /></center>
...
library(arm)
age <- arm::rescale(voteincome$age)
education  <- arm::rescale(voteincome$education)
income  <- arm::rescale(voteincome$income)
female <- voteincome$female
vote2 <- voteincome$vote

M2 <- glm(vote2 ~ age + female + education + income,
          family=binomial(link="logit"))
...
stargazer(M1, M2, type="html",
          ...
          notes="<small>Data come from <code>voteincome</code> data in <code>Zelig</code> package.</small>",
          omit.stat = c("aic","ll"),
          dep.var.caption  = "<em>Is Respondent a Voter?</em>")
```
<br /></center>
...
knitr::knit("2014-08-13-reading-a-regression-table-a-guide-for-students.Rmd",
            output = "../_posts/2014-08-13-reading-a-regression-table-a-guide-for-students.md")
```