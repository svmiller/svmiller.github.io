...
```{r setup-gun-violence-mass-shooting, include=FALSE, cache=F}
...
knitr::opts_chunk$set(fig.path = fig_path,dpi=300, echo=FALSE,
                      cache.path = '../cache/gun-violence-mass-shooting/',
...
ghp100k %>%
  ...
  group_by(country) %>%
  slice(n()) %>%
  select(country, year, ghp100k) %>%
  arrange(-ghp100k) %>%
  mutate(ghp100k = round(ghp100k, 2)) %>%
  rename(Country = country,
         `Year (Most Recent)` = year,
         `Homicide by Firearm Rate (per 100,000 People)` = ghp100k) %>%
  kable(., format="html",
...
```{r us-mass-shootings-by-year-1982-present, fig.width=13, fig.height=9}
...
knitr::knit("2019-08-03-how-bad-is-americas-gun-violence-mass-shooting-problem-in-a-few-lines-of-r-code.Rmd",
            output = "../_posts/2019-08-03-how-bad-is-americas-gun-violence-mass-shooting-problem-in-a-few-lines-of-r-code.md")
```