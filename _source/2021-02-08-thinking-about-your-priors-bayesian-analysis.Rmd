...
knitr::opts_chunk$set(fig.path = fig_path, dpi= 300,
                      cache.path = '~/Dropbox/svmiller.github.io/cache/thinking-about-priors-bayes/',
...
setwd("~/Dropbox/svmiller.github.io")
...
tidyB01 <- readRDS("R/priors/tidyB01.rds")

```

```{r leadimage, echo=F, eval=T, results="asis", cache=F}
 
add_jekyll_image('/images/prior-posterior-likelihood.png', "The intuition behind Bayesian inference.", "400", "right")
...
ESS9GB %>%
  ...
  summarize(mean = mean(val, na.rm=T),
            ...
            max = max(val, na.rm=T),
            n = n()) %>%
  mutate(sd = ifelse(var == "uempla",  sqrt(mean*(1-mean)/n), sd)) %>%
  mutate(var = c("Age", "Education (in Years)", "Female","Household Income (Deciles)",
                 "Pro-Immigration Sentiment", "Ideology (L to R)",
                 "Unemployed")) %>%
  arrange(var) %>%
  mutate_if(is.numeric, ~round(.,3)) %>%
  kable(., format="html",
        table.attr='id="stevetable"',
        col.names = c("Variable", "Mean", "Std. Dev.", "Median", "Min.", "Max.", "N"),
        caption = "Descriptive Statistics in the ESS9GB Data in {stevedata}",
...
```{r the-effect-of-a-cocksure-prior, echo=F, eval=T, cache=T, warning=F, fig.width=11, fig.height = 8}
tidyB01 %>%
  group_by(cat, var) %>%
  ...
  mutate(var = fct_reorg(var,
                         ...
                         "Female" = "b_female",
                         "Houshold Income (Deciles)" = "b_hinctnta",
                         "Ideology (L-R)" = "b_lrscale",
                         "Unemployed" = "b_uempla"),
         var = fct_rev(var),
         cat = fct_rev(cat)) %>%
  ...
  labs(x = "", y = "Mean Coefficient (with 90% Intervals)",
       title = "The Effect of a Cocksure Prior in a Bayesian Analysis",
       subtitle = "When the proportion of a dummy IV is really small, a strong prior can have a large effect on inferences.",
       caption = "Data: ?ESS9GB in {stevedata}. Intercept omitted for readability. You should really standardize your variables, which I don't do here.")
```
...
```{r standard-normal-diffuse-normal, echo=F, eval=T, cache=T, warning=F, fig.width=11, fig.height = 6}
jenny()
tibble(`Normal(0,1)` = rnorm(100000, 0, 1),
       `Normal(0,10^6)` = rnorm(100000, 0, 10^6)) %>%
  gather(var, val) %>%
  ggplot(.,aes(val)) + geom_density() +
  facet_wrap(~var, scales="free") +
  theme_steve_web() + post_bg() +
  labs(y = "", x ="",
       caption = "Data are simulated to avoid having to think about stat_function() within facet_wrap().",
       title = "A Standard Normal Distribution and a Diffuse Normal Distribution",
       subtitle = "Pay careful attention to the x-axis. The data are distributed normally, but the more diffuse distribution includes more plausible values.")
```
...
```{r students-t-priors-uniondensity, echo=F, eval=T, cache=T, warning=F, fig.width=11, fig.height = 6}
t_examples %>%
  gather(var, val) %>%
  ggplot(.,aes(val)) + geom_density() +
  facet_wrap(~var, scales="free") +
  theme_steve_web() + post_bg() +
  labs(title = "Density Plots of Various Student's t-Distributions",
       x = "", y = "",
       subtitle = "The location-scale version of Student's t-distribution has some fairly long, if not necessarily ''fat'', tails.",
       caption = "Data are simulated to avoid having to think about stat_function() within facet_wrap().")
```
...
t_examples %>%
  ...
  mutate_if(is.numeric, ~round(.,2)) %>%
   kable(., format="html",
        table.attr='id="stevetable"',
        col.names = c("Distribution", "Mean", "50% Range", "Min.", "Max.", "Number of Observations More Extreme Than -3 or 3", "% of Extreme Observations"),
        caption = "A Summary of the Extreme Values in Various Distributions (Simulated: N = 100,000)",
        align=c("l","c","c", "c", "c", "c","c","c","c","c","c"))
```
...
```{r comparing-exponential-half-t-rstanarm-brms, echo=F, eval=T, cache=T, warning=F, fig.width=11, fig.height = 6}
jenny()
tibble(`exp(.05)` = rexp(100000, rate = 1/sd(uniondensity$union)),
       `half-student_t(3,0,25.4)` = rstudent_t(100000, 3, 0, 25.4)) %>%
  gather(var, val) %>%
  filter(val >= 0) %>%
  ggplot(.,aes(val)) + geom_density() +
  facet_wrap(~var, scales="free") +
  theme_steve_web() + post_bg() +
  labs(title = "Comparing Default Priors for Sigma in a Linear Model in {brms} and {rstanarm}",
       x = "", y ="",
       subtitle = "Both distributions are weakly informative, but the exponential might be a just bit more informative (in a good way).",
       caption = "Data are simulated to avoid having to think about stat_function() within facet_wrap().")
```
...
```{r effect-various-priors-union-density, echo=F, eval=T, cache=T, warning=F, fig.width=12, fig.height = 8}

m_sums %>%
  mutate(variable = fct_reorg(variable,
                              "Left-Wing Governments" = "b_left",
                              "Industrial Concentration" = "b_concen",
                              "Logged Labor Force Size" = "b_size",
                              "Intercept" = "b_Intercept")) %>%
  ggplot(.,aes(prior, mean, ymin=lwr, ymax=upr)) + 
  geom_pointrange() +
  theme_steve_web() + post_bg() +
  geom_hline(yintercept = 0, linetype="dashed") +
  facet_wrap(~variable, scales="free") +
  coord_flip() +
  labs(x = "", y = "Mean Estimate (with 90% Intervals)",
       title = "The Effect of Various Prior Distributions on Inferences about Union Density",
       subtitle = "The standard normal prior (0,1) was a really bad choice while more reasonable ignorance priors turn out to be more informative.",
       caption = "Data: ?uniondensity in {stevedata}. See also: Western and Jackman (1994).")
...
knitr::knit("_source/2021-02-08-thinking-about-your-priors-bayesian-analysis.Rmd",
            output = "_posts/2021-02-08-thinking-about-your-priors-bayesian-analysis.md")
