...
knitr::opts_chunk$set(fig.path = fig_path, dpi= 300,
                      cache.path = '../cache/nd-clt-inference/',
...
add_jekyll_image('/images/gauss-german-mark.jpg', "Carl Friedrich Gauss, who discovered the normal distribution, honored on the 10-Deutsche Mark.", "350", "right")
...
```{r super-simple-normal-distribution, echo=T, eval=T, cache=T, warning=F, fig.width=12, fig.height = 8}
...
```{r parabola-exponentiated, echo=T, eval=T, cache=T, warning=F, fig.width=15, fig.height = 8}
...
```{r a-normal-distribution-with-areas-under-curve, echo=T, eval=T, cache=T, warning=F, fig.width=13, fig.height = 8}
...
[^wellactually]: I do love to reiterate that these are imprecise, and technically incorrect. For example, 68% of the distribution is within about `r z68` standard units from zero in a normal distribution, 90% of the distribution is within about `r z90` standard units from zero, 95% of the distribution is within about `r z95` standard units while 99% of the distribution is within about `r z99` standard units from zero in the normal distribution. Thus, these thresholds approximate the rule. It's good to be conservative but it's important to be honest. Just be mindful of that.
...
```{r thermometer-rating-donald-trump-anes-2018, echo=T, eval=T, cache=T, warning=F, fig.width=13, fig.height = 8}
...
Therms18 %>%
  select(fttrump) %>%
  na.omit %>%
  summarize(min = min(fttrump),
            max = max(fttrump),
            mode = 0,
            median = median(fttrump),
            mean = mean(fttrump),
            sd = sd(fttrump)) %>%
 # mutate_if(is.numeric, ~round(., 2)) %>%
  kable(., format="html", # make HTML table
        table.attr='id="stevetable"', # My custom CSS, to make stuff pretty
        col.names = c("Minimum", "Maximum", "Mode", "Median", "Mean",
                      "Standard Deviation"),
        caption = "Descriptive Statistics of the Thermometer Rating for Donald Trump (2018 Pilot Study, ANES)",
        align=c("c","c","c","c","c","c"))
...
tibble(min = min(Population),
       max = max(Population),
       mode = getmode(Population),
  ...
  kable(., format="html", # make HTML table
        ...
        col.names = c("Minimum", "Maximum", "Mode", "Median", "Mean",
                      "Standard Deviation"),
        caption = "Descriptive Statistics of Our Simulated Population Data",
        align=c("c","c","c","c","c","c"))
```
...
And here's the distribution of sample means. Notice the distribution of sample means (as a histogram) converges on a normal distribution where the provided location and scale parameters are from the one million sample means. Further, the center of the distribution is converging on the known population mean. The true population mean is `r mean(Population)`. The mean of the one million sample means is `r mean(Popsamples$samplemean)`. They're clearly not identical, but I also technically did not do infinity samples either. The principle is still clear from what emerges. 

```{r central-limit-theorem-example, echo=T, eval=T, cache=T, warning=F, fig.width=11, fig.height = 8}
Popsamples %>%
...
```{r diminishing-returns-of-increased-sample-size, echo=T, eval=T, cache=T, warning=F, fig.width=12, fig.height = 8}
...
```{r diminishing-returns-of-increased-sample-size-confint, echo=T, eval=T, cache=T, warning=F, fig.width=12, fig.height = 8}
Samps %>%
  group_by(sampsize, samp) %>%
...
Samps %>%
  ...
  summarize(sampmean = mean(value)) %>%
  ungroup() %>%
  spread(sampsize, sampmean) %>%
  slice(1, 3:10, 2) %>%
  mutate_if(is.numeric, ~round(.,2)) %>%
  mutate(samp = c("Sample #1", "Sample #2", "Sample #3", "Sample #4", "Sample #5",
                  "Sample #6", "Sample #7", "Sample #8", "Sample #9", "Sample #10")) %>%
    kable(., format="html", # make HTML table
        table.attr='id="stevetable"', # My custom CSS, to make stuff pretty
        col.names = c("Sample No.", "Size: 10", "Size: 25", "Size: 100", "Size: 400",
                      "Size: 1000", "Size: 2000", "Size: 4000", "Size: 10000"),
...
That fluke sample mean in the 1,000-sample group is the fourth sample mean. In this case, the sample mean from the `Population` data was ~34.89. This is nowhere close to the other sample means, which all hover close to the actual population mean of `r mean(Population)`. This leads to an interesting question: what was the probability we got that fluke sample from the `Population` data, knowing in advance the true population mean is `r mean(Population)`?
...
Toward that end, we'll need to get a *z*-score, which is simply defined in this case as the sample mean of interest to us ($$\bar{x}$$ = 34.89), minus the true population mean ($$\mu$$ = `r mean(Population)`), divided over the standard error of the sample mean (which we previously defined as ($$\frac{\sigma}{\sqrt(n)}$$).
...
Here's how you would interpret this information. Knowing in advance the true population mean is `r mean(Population)` and the standard deviation ($$\sigma$$) inherent in the population is `r sd(Population)`, the probability of us observing that sample mean of 34.89 from a random sample of 1,000 observations is `r 1-pnorm(abs(zscore))`. Look at how many zeroes precede the first number. Informally, it's a chance sample mean that would occur roughly 29 times in 1,000,000 trials, on average. That's quite a fluke occurrence! It's any wonder I got it with this reproducible seed.
...
Recall the `Population` data frame is a "population", or a universe of cases, in which the true mean thermometer rating is `r mean(Population)` even if there is a lot of variation in the data. Let's grab a random sample of 1,000 observations and see if we can approximate it.
...
This isn't bad at all. The true population mean is `r mean(Population)`. We're guessing `r mean(Samp1k)`. We're definitely in the ballpark, even if our sample mean is technically wrong.

Let's assume two situations. First, we don't know the true population mean ($$\mu$$). We only have an estimate of it ($$\bar{x}$$) from our random sample of 1,000 observations but we'll assume, for the moment, that we know the population standard deviation ($$\sigma$$). Second, let's assume we encounter a devout partisan of this hypothetical divisive public official. This devout partisan is suspicious of our---shall we say---"fake" estimate of `r mean(Samp1k)` and instead suggests the "true" population mean is 73.88. If you're curious, this is [the percentage of the vote that Trump got](https://en.wikipedia.org/wiki/2016_United_States_presidential_election_in_South_Carolina#By_county) in Pickens County, South Carolina (my home county) in the 2016 presidential election. Basically, this person is doing a ["hasty generalization"](https://www.logicallyfallacious.com/logicalfallacies/Hasty-Generalization), which is one of my favorite teachable moments for students learning about inference. S/he is making assumptions about the population based on a non-representative part of it.
...
If the true population mean were 73.88, the probability of us observing the sample mean that was `r round(zscore, 2)` standard errors away from 73.88 is effectively an impossibility. So, we reject the claim the true population mean is 73.88 and suggest our sample mean of `r mean(Samp1k)` is closer to what it truly is. In our case, we know this is true.
...
knitr::knit("2020-03-16-normal-distribution-central-limit-theorem-inference.Rmd",
            output = "../_posts/2020-03-16-normal-distribution-central-limit-theorem-inference.md")
```
