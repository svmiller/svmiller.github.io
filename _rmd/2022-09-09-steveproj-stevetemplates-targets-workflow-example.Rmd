...
add_jekyll_image('/images/soyjaks-painting-visnetwork.png', "I'm easily impressed by my own parlor tricks.", "400", "right")
```
...
tribble(~object, ~func, ~descr, ~depends,
        "Data", "prep()", "Generate finished data object(s) for analysis", "raw data (potentially shareable), assorted libraries needed for the `prep()` function",
        "Mods", "analysis()", "Generated (regression) model objects from an analysis", "`Data`, assorted libraries needed for `analysis()`",
        "QI", "qi()", "Generate (simulated) quantities of interest from an analysis.", "`Mods`, assorted libraries needed for `qi()`") %>%
  kbl(., caption = "An Object-Focused Description of an Academic Project",
      format="html",
      col.names = c("Object", "Simple (R) Function for Generating Object",
                    "Description of Object", "Dependencies"),
      table.attr='id="stevetable"',
      align = c("ccll")) %>%
  column_spec(c(1,2), monospace = TRUE)
```
...
```{r tarvisnetwork, echo=F, eval=T, results="asis", cache=F}
add_jekyll_image('/images/steveproj-tarvisnetwork.png', "A pipeline dependency graph of the academic project", "1000", "center")
```
...
```{r tarloadglobals, echo=F, eval=T, results="asis", cache=F}
add_jekyll_image('/images/steveproj-tarloadglobals.png', "tar_load_globals() loads the libraries and the functions into the current R session.", "460", "center")
```
