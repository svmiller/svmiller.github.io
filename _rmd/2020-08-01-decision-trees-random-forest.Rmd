...
```{r rpart-tree-votetrump16-midwest, echo=T, eval=T, warning=F, fig.width=12, fig.height = 6}
...
```{r illustration-of-variable-importance-trump-vote-2016, echo=F, eval=T, warning=F, fig.width=12, fig.height = 8}
importance_tab %>%
  group_by(var) %>%
...
accuracyStates %>%
  ...
  left_join(., usa_states %>% select(statename, division, region) %>% rename(state = statename)) %>%
