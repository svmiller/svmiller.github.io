...
Everything <- readRDS("~/Dropbox/svmiller.github.io/R/clt-nd-inference/Everything.rds")

set.seed(8675309)
one_off_sample_mean <- mean(sample(Everything$Population, 10, replace=F))
# ^ Note: random seed generation changed from R 3.5 (which I used originally) to  R 4.0.
# I had to find everything from a chunk to ensure things mostly stayed unchanged.
# If you do this in R 4.0, things will slightly change but the underlying principles don't/won't.
```
...
```{r parabola-exponentiated, echo=T, eval=T,  warning=F, fig.width=15, fig.height = 8}
...
[^wellactually]: I do love to reiterate that these are imprecise, and technically incorrect. For example, 68% of the distribution is within about `r qnorm(.16, lower.tail = F)` standard units from zero in a normal distribution, 90% of the distribution is within about `r qnorm(.05, lower.tail = F)` standard units from zero, 95% of the distribution is within about `r qnorm(.025, lower.tail = F)` standard units while 99% of the distribution is within about `r qnorm(.005, lower.tail = F)` standard units from zero in the normal distribution. Thus, these thresholds approximate the rule. It's good to be conservative but it's important to be honest. Just be mindful of that.
...
tibble(min = min(Everything$Population),
       max = max(Everything$Population),
       mode = getmode(Everything$Population),
       median = median(Everything$Population),
       mean = mean(Everything$Population),
       sd = sd(Everything$Population)) %>%
  kable(., format="html", # make HTML table
        table.attr='id="stevetable"', # My custom CSS, to make stuff pretty
        col.names = c("Minimum", "Maximum", "Mode", "Median", "Mean",
                      "Standard Deviation"),
...
And here's the distribution of sample means. Notice the distribution of sample means (as a histogram) converges on a normal distribution where the provided location and scale parameters are from the one million sample means. Further, the center of the distribution is converging on the known population mean. The true population mean is `r mean(Everything$Population)`. The mean of the one million sample means is `r mean(Everything$Popsamples$samplemean)`. They're clearly not identical, but I also technically did not do infinity samples either. The principle is still clear from what emerges. 

```{r, echo=T, eval=F, warning=F, fig.width=11, fig.height = 8}
Popsamples %>%
...
```{r central-limit-theorem-example, echo=F, eval=T, fig.width=11, fig.height = 8}
Everything$Popsamples %>%
  ggplot(.,aes(samplemean)) + 
  geom_histogram(binwidth=.5,aes(y=..density..),alpha=0.7,fill="#9BB2CE") +
  theme_steve_web() + 
  geom_vline(xintercept = mean(Everything$Population), linetype="dashed") +
  stat_function(fun=dnorm,
                color="#002F5F", size=1.5,
                args=list(mean=mean(Everything$Popsamples$samplemean), 
                          sd=sd(Everything$Popsamples$samplemean))) +
  labs(x = "Sample Mean", y = "Density",
       title = "The Distribution of 1,000,000 Sample Means, Each of Size 10",
       subtitle = "Notice the distribution is normal and the mean of sample means converges on the known population mean (vertical line).",
       caption = "Data: Simulated data from blog post at svmiller.com.")
...
```{r, echo=T, eval=F, fig.width=12, fig.height = 8}
...
```
```{r diminishing-returns-of-increased-sample-size, echo=F, eval=T, fig.width=12, fig.height = 8}
Everything$Samps %>%
  group_by(sampsize, samp) %>%
  summarize(sampmean = mean(value)) %>%
  ggplot(., aes(as.factor(sampsize),sampmean)) + 
  geom_point(size=3, color="black", alpha=0.5) +
  theme_steve_web() + 
  geom_hline(yintercept = mean(Everything$Population), linetype="dashed") +
  labs(x = "Sample Size",
       y = "Sample Means",
       title = "Ten Sample Means of Varying Sample Sizes from a Population",
       subtitle = "The diminishing returns of increasing sample size emerge around 1,000 observations, even as the spread in these simulated data is quite large.",
...
```{r, echo=T, eval=F, warning=F, fig.width=12, fig.height = 8}
Samps %>%
  group_by(sampsize, samp) %>%
...
```
```{r diminishing-returns-of-increased-sample-size-confint, echo=F, eval=T,  warning=F, fig.width=12, fig.height = 8}
Everything$Samps %>%
  group_by(sampsize, samp) %>%
  mutate(sampmean = mean(value),
         se = sd(Population)/sqrt((sampsize)),
...
That fluke sample mean in the 1,000-sample group is the fourth sample mean. In this case, the sample mean from the `Population` data was ~34.89. This is nowhere close to the other sample means, which all hover close to the actual population mean of `r mean(Everything$Population)`. This leads to an interesting question: what was the probability we got that fluke sample from the `Population` data, knowing in advance the true population mean is `r mean(Everything$Population)`?
...
Toward that end, we'll need to get a *z*-score, which is simply defined in this case as the sample mean of interest to us ($$\bar{x}$$ = 34.89), minus the true population mean ($$\mu$$ = `r mean(Everything$Population)`), divided over the standard error of the sample mean (which we previously defined as ($$\frac{\sigma}{\sqrt(n)}$$).
...
Here's how you would interpret this information. Knowing in advance the true population mean is `r mean(Everything$Population)` and the standard deviation ($$\sigma$$) inherent in the population is `r sd(Everything$Population)`, the probability of us observing that sample mean of 34.89 from a random sample of 1,000 observations is `r formatC(pval, digits=6,format="f")`. Look at how many zeroes precede the first number. Informally, it's a chance sample mean that would occur roughly 29 times in 1,000,000 trials, on average. That's quite a fluke occurrence! It's any wonder I got it with this reproducible seed.
...
mean(Everything$Samp1k)
```

This isn't bad at all. The true population mean is `r mean(Everything$Population)`. We're guessing `r mean(Everything$Samp1k)`. We're definitely in the ballpark, even if our sample mean is technically wrong.
