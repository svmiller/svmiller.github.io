...
*Last updated: `r format(Sys.Date(), "%d %B %Y")`. Some things got lost in transition behind the scenes. These include a move from R version 3.5 to 4, a transition from working directories, and some function changes in `{stevemisc}`.*
...
wages12 %>%
  ...
  mutate(perc = ifelse(!is.na(perc), paste0(mround(perc),"%"), NA))  %>%
  kable(., format="html",
        table.attr='id="stevetable"',
